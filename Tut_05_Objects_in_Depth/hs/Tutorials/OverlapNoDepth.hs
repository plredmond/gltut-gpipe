...
main = do
    ...
    Framework.main keyboard
                   (displayIO [ (VecFile.readIndexedStream dat_h indexes, toGPU (0:.0:.0:.0:.()))
...
displayIO :: [Object] -> Vec2 Int -> IO (FrameBuffer RGBFormat () ())
...
display :: [Object] -> Vec2 Int -> Float -> FrameBuffer RGBFormat () ()
display objects size sec = draw fragments cleared
    where
        ...
        tweak = let z = (-1.1) * Easing.computeCycle 5 sec
                 in toGPU (0:.0:.z:.0:.())
