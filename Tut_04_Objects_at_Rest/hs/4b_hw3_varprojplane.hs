...
--                  $ fmap (vs_applypos $ (\p -> mat `multmv` p))
                  $ fmap (vs_applypos $ vs_manualpersp projplane)
...
vs_manualpersp :: Float3v -> Posv -> Posv
vs_manualpersp (pplX:.pplY:.pplZ:.()) (camX:.camY:.camZ:.camW:.()) = clipPos
    where
        pr1 = (zNear + zFar) / (zNear - zFar)
        pr2 = (2 * zNear * zFar) / (zNear - zFar)
        clipPos = (camX * frustrumScale + pplX) :.
                  (camY * frustrumScale + pplY) :.
                  (camZ * pr1 + pr2) :.
                  (camZ / negate pplZ) :. ()
