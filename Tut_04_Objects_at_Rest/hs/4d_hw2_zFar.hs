...
--                  (GL.PixelData GL.RGB GL.Float buffer)
    ...
    let ppmdl = toGPUStream TriangleFan [
        ...
        ppfrg = fmap (\_ -> RGBA (vec 1) 0.5)
              $ rasterizeFront
              $ fmap (\p -> (p, ()))
              $ fmap (vs_matrixpersp $ vs_projmat (Vec.map fromIntegral size) 1.0 0.5 3.0)
              ppmdl
...
--                  $ fmap (vs_applypos $ vs_morepowertotheshaders 5 time)
        ...
        -- mat
        mat = vs_projmat size 1.0 0.5 (pop * 3.0) -- 0.5 3.0
...
vs_projmat :: Sizv -> Float_v -> Float_v -> Float_v -> Matrixv -- Mat44 Float_v
vs_projmat size scale zNear zFar = 
    (((scx):.(  0):.(  0):.(  0):.()):.
     ((  0):.(scy):.(  0):.(  0):.()):.
     ((  0):.(  0):.(pr1):.(pr2):.()):.
     ((  0):.(  0):.(neg):.(  0):.()):.
