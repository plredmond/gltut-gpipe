...
renderFrame mdl size = do
    ...
    return $ let gpusize = toGPU $ Vec.map fromIntegral size
                 gputime = toGPU $ fromIntegral time / 1000
             in  drawFrame mdl gputime gpusize
...
--                  $ fmap (vs_applypos $ (\p -> mat `multmv` p))
                  $ fmap (vs_applypos vs_orthowoffset)
                  mdl
...
-- offset uniform
offset :: Float2v
offset = toGPU 0.5:.0.5:.()

-- apply offset uniform to pos
vs_orthowoffset :: Posv -> Posv
vs_orthowoffset pos = pos + (append offset (0:.0:.()))
