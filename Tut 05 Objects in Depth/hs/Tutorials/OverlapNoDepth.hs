...
displayIO :: [(Models.PrimStream, Vec4 (Vertex Float))] -> Vec2 Int -> IO (FrameBuffer RGBFormat () ())
...
display :: [(Models.PrimStream, Vec4 (Vertex Float))] -> Vec2 Int -> Float -> FrameBuffer RGBFormat () ()
display objects size sec = draw fragments cleared
    where
        ...
        tweak = let z = Easing.lerpThereAndBack sec 5 `Easing.onRange` (-1, 0)
                 in toGPU (0:.0:.z:.0:.())
